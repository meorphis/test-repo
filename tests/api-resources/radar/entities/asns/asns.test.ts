...
describe('resource asns', () => {
  ...
  test.skip('retrieve', async () => {
    const responsePromise = cloudflare.radar.entities.asns.retrieve(3);
  ...
  test.skip('retrieve: request options instead of params are passed correctly', async () => {
    ...
    await expect(
      cloudflare.radar.entities.asns.retrieve(3, { path: '/_stainless_unknown_path' }),
  ...
  test.skip('retrieve: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      cloudflare.radar.entities.asns.retrieve(3, { format: 'JSON' }, { path: '/_stainless_unknown_path' }),
    ).rejects.toThrow(Cloudflare.NotFoundError);
  });
