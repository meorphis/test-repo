...
export class ManagedRules extends APIResource {
  /**
   * Percentage distribution of attacks by managed rules used over time.
   */
  listTimeseriesGroups(
    query?: ManagedRuleListTimeseriesGroupsParams,
    options?: Core.RequestOptions,
  ): Core.APIPromise<ManagedRuleListTimeseriesGroupsResponse>;
  listTimeseriesGroups(
    options?: Core.RequestOptions,
  ): Core.APIPromise<ManagedRuleListTimeseriesGroupsResponse>;
  listTimeseriesGroups(
    query: ManagedRuleListTimeseriesGroupsParams | Core.RequestOptions = {},
    options?: Core.RequestOptions,
  ): Core.APIPromise<ManagedRuleListTimeseriesGroupsResponse> {
    if (isRequestOptions(query)) {
      return this.listTimeseriesGroups({}, query);
    }
    return this._client.get('/radar/attacks/layer7/timeseries_groups/managed_rules', { query, ...options });
  }
}

export interface ManagedRuleListTimeseriesGroupsResponse {
  result: ManagedRuleListTimeseriesGroupsResponse.Result;

  success: boolean;
}

export namespace ManagedRuleListTimeseriesGroupsResponse {
...
export interface ManagedRuleListTimeseriesGroupsParams {
  /**
