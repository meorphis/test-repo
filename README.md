# Meorphis Test 1 Node API Library

[![NPM version](https://img.shields.io/npm/v/meorphis-test-1.svg)](https://npmjs.org/package/meorphis-test-1)
...
const meorphisTest1 = new MeorphisTest1({
  apiKey: process.env['MEORPHIS_TEST_1_API_KEY'], // This is the default and can be omitted
  environment: 'environment_1', // defaults to 'production'
});

async function main() {
  const statusRetrieveResponse = await meorphisTest1.status.retrieve();
...
async function main() {
  const statusRetrieveResponse: MeorphisTest1.StatusRetrieveResponse = await meorphisTest1.status.retrieve();
}
...
const response = await meorphisTest1.status.retrieve().asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: statusRetrieveResponse, response: raw } = await meorphisTest1.status.retrieve().withResponse();
...
add the following import before your first import `from "MeorphisTest1"`:
...
- Deno v1.28.0 or higher, using `import MeorphisTest1 from "npm:meorphis-test-1"`.
- Bun 1.0 or later.
- Cloudflare Workers.
- Vercel Edge Runtime.
